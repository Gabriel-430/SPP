...
public class EvaluarReporteFinalController implements Initializable {
    ...
    private void cargarReportes() {
        try {
            ...
            List<Reporte> lista = reporteService.recuperarReportesPendientes(idUsuario, "Reporte Final");

            cmbReportes.setItems(FXCollections.observableArrayList(lista));

            if (lista.isEmpty()) {
                Alerta.mostrarAlertaInformacion("Bandeja vacia", "No hay reportes finales pendientes por evaluar.");
            } else {
    ...
    void clicAgregarObservacion(Event event) {
        AgregarObservacionController.reporteOrigen = AgregarObservacionController.REPORTE_FINAL;
